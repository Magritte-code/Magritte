...
def image_mpl(
    ...
    sorted_freq_range = np.argsort(freqs)
    f_ij  = np.mean(freqs)
    ...
    for f in tqdm(sorted_freq_range): #in order to add the images in a sorted order
...
def image_plotly(
    ...
    # Build up plot
    for f in sorted_freq_range: #in order to add the images in a sorted order
        fig.add_trace(
            go.Heatmap(
                x          = xs       .astype(float),
                y          = ys       .astype(float),
