...
accel inline Real Solver ::get_dshift_max(const Model& model, const Size o) {
    ...
    for (const LineProducingSpecies& lspec : model.lines.lineProducingSpecies) {
        ...
        const Real new_dshift_max =
            model.parameters->max_width_fraction * model.thermodynamics.profile_width(inverse_mass, o);
...
inline Size Solver ::get_ray_lengths_max_new_imager(Model& model, Image& image, const Vector3D& ray_dir) {
    ...
    ray_lengths.resize(npixels); // TODO: later on, define which pixels need to be used for raytracing (adaptive
                                 // imager)/or define new partial images (might be more elegant)
...
//     if (!model.parameters->store_intensities)
//     {
//         model.radiation.u.resize (model.parameters->hnrays(), model.parameters->npoints(),
//         model.parameters->nfreqs()); model.radiation.v.resize (model.parameters->hnrays(),
//         model.parameters->npoints(), model.parameters->nfreqs());
//     }
//
//
//     // For each ray, solve transfer equation
...
template <ApproximationType approx> inline void Solver ::solve_feautrier_order_2_sparse(Model& model) {
            ...
            threaded_for (o, model.parameters->npoints(), {
                ...
                if (n_tot_() > 1) {
                    ...
                    for (Size k = 0; k < lspec.linedata.nrad; k++) {
                        // Integrate over the line
                        for (Size z = 0; z < model.parameters->nquads(); z++) {
                            lspec.J(o, k) += lspec.quadrature.weights[z] * wt
                                           * boundary_intensity(
                                               model, o, model.radiation.frequencies.nu(o, lspec.nr_line[o][k][z]));
                        }
                    }
                }
            })
...
template <ApproximationType approx> inline void Solver ::solve_feautrier_order_2_anis(Model& model) {
    // Initialise variables
    for (LineProducingSpecies& lspec : model.lines.lineProducingSpecies) {
        lspec.J.resize(model.parameters->npoints(), lspec.linedata.nrad);
            ...
            threaded_for (o, model.parameters->npoints(), {
                ...
                if (n_tot_() > 1) {
                    ...
                    for (Size k = 0; k < lspec.linedata.nrad; k++) {
                        // Integrate over the line
                        for (Size z = 0; z < model.parameters->nquads(); z++) {
                            const Real du = lspec.quadrature.weights[z] * wt
                                          * boundary_intensity(
                                              model, o, model.radiation.frequencies.nu(o, lspec.nr_line[o][k][z]));
...
template <ApproximationType approx> inline void Solver ::image_feautrier_order_2(Model& model, const Size rr) {
        ...
        if (n_tot_() > 1) {
            for (Size f = 0; f < model.parameters->nfreqs(); f++) {
                ...
                image.I(o, f) = two * Su_()[last_()]
                              - boundary_intensity(model, nr_()[last_()], model.radiation.frequencies.nu(o, f));
            }
        } else {
            for (Size f = 0; f < model.parameters->nfreqs(); f++) {
...
inline void Solver ::image_feautrier_order_2_new_imager(
    ...
    // Note: number of pixels is constant for now, but may be adaptive in the future; (but then while loop will be
    // required anyway)
    ...
    accelerated_for (pixidx, npixels, {
        ...
        shift_()[centre] =
            model.geometry.get_shift<Rest>(origin, origin_velocity, ray_dir, closest_bdy_point, Z, false);
        first_() = trace_ray_imaging<Rest>(
                       model.geometry, origin, closest_bdy_point, ray_dir, dshift_max, -1, Z, centre - 1, centre - 1)
                 + 1;
        ...
        if (n_tot_() > 1) {
            for (Size f = 0; f < model.parameters->nfreqs(); f++) {
                ...
                image.I(pixidx, f) =
                    two * Su_()[last_()]
                    - boundary_intensity(model, nr_()[last_()], model.radiation.frequencies.nu(closest_bdy_point, f));
            }
        } else {
            for (Size f = 0; f < model.parameters->nfreqs(); f++) {
                image.I(pixidx, f) =
                    boundary_intensity(model, closest_bdy_point, model.radiation.frequencies.nu(closest_bdy_point, f));
            }
        }
    })
...
template <ApproximationType approx> inline void Solver ::image_optical_depth(Model& model, const Size rr) {
    Image image = Image(model.geometry, model.radiation.frequencies, Intensity, rr);
...
// Because of the new method for computing the optical depth, adding extra frequency points for counteracting the large
// doppler shift is no longer necessary
template <Frame frame>
accel inline Size Solver ::trace_ray(const Geometry& geometry, const Size o, const Size r, const double dshift_max,
...
// Because of the new method for computing the optical depth, adding extra frequency points for counteracting the large
// doppler shift is no longer necessary Specialized raytracer for imaging the model. Note: assumes the outer boundary to
// be convex in order to correctly start tracing the ray
accel inline Size Solver ::trace_ray_imaging_get_start(
    ...
    // TODO: is slightly inefficient implementation, can be improved by only checking bdy point neighbors
    // We use here the assumption that the outer boundary is convex, to obtain the closest point on the boundary
    while (true) {
...
// be convex in order to correctly start tracing the ray
template <Frame frame>
accel inline Size Solver ::trace_ray_imaging(const Geometry& geometry, const Vector3D& origin, const Size start_bdy,
    const Vector3D& raydir, const double dshift_max, const int increment,
    ...
    if (geometry.valid_point(nxt)) {
        ...
        // Due to boundary points begin slightly annoying to start a ray from, we must make sure the ray only end when
        // we are sure that we stay on the boundary
        while (true) {
            if (!geometry.not_on_boundary(nxt)) {
                ...
                while (true) {
                    ...
                    if ((!geometry.valid_point(temp_nxt)) || (curr_cons_bdy == MAX_CONSECUTIVE_BDY)) {
                        return id1; // the ray ends if we cannot find any points anymore, or we have too many boundary
                                    // points after eachother
                    }
...
// function corresponds to a combination of trace_ray_imaging_get_start and trace_ray_imaging, deleting all unneccesary
// computations (shift) and replacing the data setting with incrementing the ray length.
        ...
        while (true) {
            if (!geometry.not_on_boundary(nxt)) {
                ...
                while (true) {
                    temp_nxt = geometry.get_next<Imagetracer>(origin, raydir, temp_nxt, temp_Z, temp_dZ);
                    if ((!geometry.valid_point(temp_nxt)) || (curr_cons_bdy == MAX_CONSECUTIVE_BDY)) {
                        return l; // the ray ends if we cannot find any points anymore, or we have too many boundary
                                  // points after eachother
                    }
                    if (geometry.not_on_boundary(temp_nxt)) {
                        break; // the ray continues, as we find once again non-boundary points
...
// Because of the new method for computing the optical depth, adding extra frequency points for counteracting the large
// doppler shift is no longer necessary
accel inline void Solver ::set_data(const Size crt, const Size nxt, const double shift_crt, const double shift_nxt,
...
accel inline void Solver ::get_eta_and_chi<CloseLines>(const Model& model, const Size p,
    ...
    const Real upper_bound_line_width =
        ...
        * model.thermodynamics.profile_width_upper_bound_with_linefreq(p, freq, model.lines.max_inverse_mass);
    ...
    auto left_line_bound =
        std::lower_bound(model.lines.sorted_line.begin(), model.lines.sorted_line.end(), left_freq_bound);
    auto right_line_bound =
        std::upper_bound(model.lines.sorted_line.begin(), model.lines.sorted_line.end(), right_freq_bound);
...
///////////////////////////////////////////////////////
accel inline Real trap(const Real x_crt, const Real x_nxt, const double dZ) { return half * (x_crt + x_nxt) * dZ; }
...
accel inline void Solver ::solve_shortchar_order_0(Model& model, const Size o, const Size r) {
    ...
    if (model.geometry.valid_point(nxt)) {
        ...
        for (Size f = 0; f < model.parameters->nfreqs(); f++) {
            ...
            compute_source_dtau<approx>(model, crt, nxt, l, freq * shift_c, freq * shift_n, shift_c, shift_n, dZ,
                compute_curr_opacity, dtau, chi_c[f], chi_n[f], source_c[f], source_n[f]);
            ...
            // const Real lambda_factor = (dtau+expm1f(-dtau))/dtau;// If one wants to compute lambda a bit more
            // accurately in case of dtau≃0. Real L   = constante * freq * phi * lambda_factor * inverse_chi;
            Real L = constante * freq * phi * (factor + 1.0)
                   * inverse_chi; // using factor+1.0, the computed lambda elements can be negative if dtau very small;
                                  // but then the lambda elements are also negligible
        ...
        //  bool prev_compute_curr_opacity=compute_curr_opacity;//technically, we could also keep this bool individually
        //  for every frequency
        prev_compute_curr_opacity =
            compute_curr_opacity; // technically, we could also keep this bool individually for every frequency
        ...
        while (model.geometry.not_on_boundary(nxt)) {
            ...
            for (Size f = 0; f < model.parameters->nfreqs(); f++) {
                ...
                model.radiation.I(r, o, f) +=
                    expf(-tau[f])
                    * (expm1f(-dtau) / dtau * (source_c[f] - source_n[f] * (1.0 + dtau)) + source_c[f] - source_n[f]);
                // TODO: check order of addition, as we might be starting with the largest contributions, before adding
                // the smaller ones...
                tau[f] += dtau;
            }
...
//             const Size l    = model.radiation.frequencies.corresponding_line[f];
//
//             compute_curr_opacity = prev_compute_curr_opacity; // for the first point, we need to compute both the
//             curr and next opacity (and source)
//             // compute_curr_opacity = true; // for the first point, we need to compute both the curr and next opacity
//             (and source)
//
//             compute_source_dtau<None>(model, crt, nxt, l, freq*shift_c, freq*shift_n, shift_c, shift_n, dZ[idx-1],
//             compute_curr_opacity, dtau, chi_c[f], chi_n[f], source_c[f], source_n[f]); dtau =
...
//     Real term_c, term_n, dtau;
//     bool compute_curr_opacity, prev_compute_curr_opacity;
//     prev_compute_curr_opacity=true; // for the first point, we need to compute both the curr and next opacity (and
//     source)
//
//     // Set boundary condition
//     for (Size f = 0; f < model.parameters->nfreqs(); f++)
...
//             (and source)
//
//             compute_source_dtau<None>(model, crt, nxt, l, freq*shift_c, freq*shift_n, shift_c, shift_n,
//             dZ[indexp1-1], compute_curr_opacity, dtau, chi_c[f], chi_n[f], source_c[f], source_n[f]); dtau =
//             std::max(model.parameters->min_dtau, dtau);
//
//             // model.radiation.I(r,o,f) = exp(-dtau)*model.radiation.I(r,o,f)
//             //                          + term_c * (-expm1(-dtau)-dtau*expf(-dtau)) /dtau
...
accel inline void Solver ::update_Lambda(Model& model, const Size rr, const Size f) {
    ...
    if (freqs.appears_in_line_integral[f]) {
        ...
        //  Real inverse_opacity = 1.0/model.lines.opacity (nr[centre], k); //Inverse line opacity; includes
        //  1/HH_OVER_FOUR_PI Real L   = constante * L_diag[centre] * inverse_opacity;
        Real eta, chi; // eta is dummy var
        ...
        for (long m = 0; (m < n_off_diag) && (m + 1 < n_tot); m++) {
            if (centre >= first + m + 1) // centre-m-1 >= first
            {
                ...
                //  inverse_opacity = 1.0/model.lines.opacity (nr[n], k); //Inverse line opacity; no longer
                //  1/HH_OVER_FOUR_PI L   = constante * L_lower(m,n) * inverse_opacity;
            ...
            if (centre + m + 1 <= last) // centre+m+1 < last
            {
                ...
                // TODO: approximation if we do not have overlapping lines
                //  inverse_opacity = 1.0/model.lines.opacity (nr[n], k); //Inverse line opacity; no includes
                //  1/HH_OVER_FOUR_PI L   = constante * L_upper(m,n) * inverse_opacity;
                get_eta_and_chi<approx>(model, nr[n], k, freq_line, eta, chi);
                Real inverse_chi = 1.0 / chi;
...
///  For high velocity increments however, this does not need any extra interpolation points (-> way faster) (and some
///  extra optimizations are made in the limit of extremely large doppler shift (as erf goes to its limit value)
...
/////////////////////////////////////////////////////////////////////
template <>
inline void Solver ::compute_S_dtau_line_integrated<OneLine>(Model& model, Size currpoint, Size nextpoint, Size lineidx,
    ...
    Scurr = model.lines.emissivity(currpoint, lineidx)
          / (model.lines.opacity(currpoint, lineidx) + model.parameters->min_opacity); // current source
    ...
    // note: due to interaction with dtau when computing all sources individually, we do need to recompute Scurr and
...
/////////////////////////////////////////////////////////////////////
template <>
inline void Solver ::compute_S_dtau_line_integrated<None>(Model& model, Size currpoint, Size nextpoint, Size lineidx,
    Real currfreq, Real nextfreq, Real dZ, Real& dtau, Real& Scurr, Real& Snext) {
    Real sum_dtau             = 0.0;
...
///  In case of low velocity differences, almost two times slower
///  For high velocity increments however, this does not need any extra interpolation points (-> way faster) (and some
///  extra optimizations are made in the limit of extremely large doppler shift (as erf goes to its limit value) This
///  function only takes into account the nearby lines, saving some computation time
///    @param[in] curr_point : index of current point
///    @param[in] next_point : index of next point
///    @param[in] lineidx : index of line to integrate over
///    @param[in] currfreq : frequency at current point (in comoving frame)
...
/////////////////////////////////////////////////////////////////////
template <>
inline void Solver ::compute_S_dtau_line_integrated<CloseLines>(Model& model, Size currpoint, Size nextpoint,
    ...
    // Note: 0 source functions can be returned if no lines are nearby; but then the negligible lower bound gets
    // returned
...
///    @param[in/out] compute_curr_opacity: for deciding whether we need to compute the current opacity when using the
///    trapezoidal rule
...
template <ApproximationType approx>
accel inline void Solver ::compute_source_dtau(Model& model, Size currpoint, Size nextpoint, Size line, Real curr_freq,
    ...
    if (using_large_shift) {
        ...
        if (compute_curr_opacity) // fancy computation does not compute the current opacity, so we might need to
                                  // recompute it here
        {
...
accel inline void Solver ::solve_feautrier_order_2(Model& model, const Size o, const Size f) {
    ...
    compute_source_dtau<approx>(model, nr[first], nr[first + 1], l, freq * shift[first], freq * shift[first + 1],
        shift[first], shift[first + 1], dZ[first], compute_curr_opacity, dtau_n, chi_c, chi_n, term_c, term_n);
    ...
    for (Size n = first + 1; n < last; n++) {
        ...
        compute_source_dtau<approx>(model, nr[n], nr[n + 1], l, freq * shift[n], freq * shift[n + 1], shift[n],
            shift[n + 1], dZ[n], compute_curr_opacity, dtau_n, chi_c, chi_n, term_c, term_n);
...
inline Real Solver ::compute_dtau_single_line(
    ...
    const Real average_inverse_line_width =
        (model.lines.inverse_width(curridx, lineidx) + model.lines.inverse_width(nextidx, lineidx)) / 2.0;
    ...
    // if frequencies are equal, division by zero (due to the optical depth formula) happens if we were not to use this
    // branch
    ...
    // In this way, the diff_pos can be computed quite simple, and we do not have a discrepancy between the
    // interpolation and the bounds
    const Real diff_pos = next_pos - curr_pos;

    /// the more correct approach, taking into account also the line opacity change; however, it does not make too much
    /// of a difference in the actual result and is quite a bit slower
    // const Real delta_opacity=(next_line_opacity-curr_line_opacity);
    // const Real deltanu=-next_freq+curr_freq;//differences in curr freqs; +-1 due to shift being defined in the other
    // direction
    //
...
accel inline void Solver ::image_feautrier_order_2_for_point_loc(Model& model, const Size o, const Size f) {
    ...
    //  this is due to the weighting by the line optical depths; this might not be saved TODO think whether this is
    //  correct
...
//     Vector<Real>& FI = FI_();
//
//     bool compute_curr_opacity = true; // for the first point, we need to compute both the curr and next opacity (and
//     source)
//
//     compute_source_dtau<approx>(model, nr[first], nr[first+1], l, freq*shift[first], freq*shift[first+1],
//     shift[first], shift[first+1], dZ[first], compute_curr_opacity, dtau_n, chi_c, chi_n, term_c, term_n);
...
//          chi_c =  chi_n;
//
//         compute_source_dtau<approx>(model, nr[n], nr[n+1], l, freq*shift[n], freq*shift[n+1], shift[n], shift[n+1],
//         dZ[n], compute_curr_opacity, dtau_n, chi_c, chi_n, term_c, term_n);
//
//         const Real dtau_avg = half * (dtau_c + dtau_n);
//         inverse_A[n] = dtau_avg * dtau_c;
//         inverse_C[n] = dtau_avg * dtau_n;
