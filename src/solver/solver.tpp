...
inline void Solver ::setup_comoving_local_approx(Model& model) {
    ...
    for (Size i = 0; i < pc::multi_threading::n_threads_avail(); i++) {
        ...
        // less (in full solver, we use matrices, here vectors suffice)
        ...
        // cma_dIdnu_curr_(i).resize(width); explicit, computed intensity derivative with respect to
        // frequency; not used
...
///  rays should be symmetric, we only need to keep track of one direction
...
// ERR, should just iterate over all frequencies, ignoring those different splits. Boundary
// conditions is_upward_disc denotes whether we want an upward discretization nextpointonrayindex
// should denote how far the next point lies on the ray currpointonrayindex should do the same, but
...
///  Is necessary for determining the boundary conditions (we require to know which lines overlap to
///  determine those)
...
///  Also requires the shift, as we will use these computed ranges to determine overlap with
///  boundary frequencies
// TODO: USING THE SINGLE LINE APPROX, THIS CAN BE SIMPLIFIED ENORMOUSLY (just use all individual
// line quadratures)
...
///  Sets all implicit boundary conditions, using the computed frequency ranges for currpoint and
///  overlaps for nextpoint Internal function for the (non-approximate) comoving solver Uses the
///  line ranges computed by get_line_ranges to figure out which frequencies (at the next point) lie
///  out of bounds
...
inline void Solver ::match_overlapping_boundary_conditions(Model& model, const Size currpoint,
    ...
    while (it != multimap_freq_to_bdy_index.end() && curr_range_index < nb_ranges) {
        ...
        // In this section: I have implemented a few different options for interpolating the
        // boundary conditions
        ...
        delta_tau_()(bdy_point_on_ray_idx, bdy_freq_idx) =
            model.parameters->comoving_min_dtau; // should be small enough, but not small enough to
                                                 // crash my solver (due to /Δτ^2 necessary)
...
inline void Solver ::set_initial_boundary_conditions(Model& model, const Size initial_bdy,
    ...
    while (it != multimap_freq_to_bdy_index.end()) {
        ...
        // get far better results in case of extreme velocity gradients (and limited quadrature
        // points) However, this does not protect against using far too large doppler shifts...
        // (when going back and forth, might jump into a non-computed part; however then +- bdy
        // intensity applies either way?)
...
///  Computes the setup and the boundary conditions for the comoving solver for the forward ray
///  direction Internal function of the (non-approximate) comoving solver Assumes we have already
///  traced the ray and maps all data required for the actual computation
// TODO: do MORE specialization for the single line approx (taking inspiration from far too complex
// deque stuff?)
...
inline void Solver ::comoving_ray_bdy_setup_forward(Model& model, Size last_interesting_rayposidx) {
    ...
    // from first_+1 to last_interesting_rayposidx index, trace the ray in reverse direction to
    ...
    // For determining the boundary conditions, we are going backwards; this seems strange, but is a
    // result of an easier treatment of the spatial boundary conditions at the end
    while (rayposidx > first_()) {
        ...
        for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
            ...
            // TODO: figure out a way to only selectively need to compute the chi's twice (for each
        ...
        // for the next point, we need to know whether to compute the opacity
        // TODO: replace with more correct statement, as this only check the last frequency
        ...
        if (is_upward_disc) {
            ...
            // computation (should definitely bdy conditions) NOTE: The solver cannot deal with
            ...
            // Do not forget to exclude the last two frequencies, as these cannot have any freq der
            // computation (should definitely bdy conditions) Assumes nfreqs>=3, as otherwise, we
            // only have boundary frequencies
    ...
    // frequencies near any line,
    //  so we will use initial boundary conditions computed at the first point to put on the ray
    // note: we use the first point on the ray for determining the boundary intensity, as the
    // boundary condition originates from that point
    set_initial_boundary_conditions(
        model, nr_()[first_index], shift_first, multimap_freq_to_bdy_index);
}

///  Computes the setup and the boundary conditions for the comoving solver for the backward ray
///  direction Internal function for the (non-approximate) comoving solver Assumes we have already
///  traced the ray and maps all data required for the actual computation
// TODO: do MORE specialization for the single line approx (taking inspiration from far too complex
// deque stuff?)
template <ApproximationType approx>
inline void Solver ::comoving_ray_bdy_setup_backward(
    ...
    // frequencies near any line,
    //  so we will use initial boundary conditions computed at the first point to put on the ray
    // note: nr_()[first_index] should be point on boundary of domain; in this way, we can simply
    // add the corresponding boundary conditions
    set_initial_boundary_conditions(
        model, nr_()[first_index], shift_first, multimap_freq_to_bdy_index);
}

// Note: single line approximation might be useful for determining the splits. IMPLEMENT FANCY
// VERSION
...
/// Note: might suffer a bit from spatial inaccuracies, as the traced rays might not go exactly
...
inline void Solver ::solve_comoving_single_step(Model& model, const Size rayposidx,
    ...
    for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
        ...
        // std::cout<<"curr freq der coef 1: "<<dIdnu_coef1_curr(rayposidx,
        // next_freq_idx)<<std::endl; std::cout<<"curr freq der coef 2:
        // "<<dIdnu_coef2_curr(rayposidx, next_freq_idx)<<std::endl; std::cout<<"curr freq der coef
        // 3: "<<dIdnu_coef3_curr(rayposidx, next_freq_idx)<<std::endl;
        // std::cout<<"curr_point_on_ray_index: "<<curr_point_on_ray_index<<std::endl;
        // std::cout<<"respective freq index: "<<dIdnu_index1_curr(rayposidx,
        // next_freq_idx)<<std::endl; std::cout<<"intensity coef 1:
        // "<<intensities(curr_point_on_ray_index, dIdnu_index1_curr(rayposidx,
        // next_freq_idx))<<std::endl; std::cout<<"intensity coef 2:
        // "<<intensities(curr_point_on_ray_index, dIdnu_index2_curr(rayposidx,
        // next_freq_idx))<<std::endl; std::cout<<"intensity coef 3:
        // "<<intensities(curr_point_on_ray_index, dIdnu_index3_curr(rayposidx,
        // next_freq_idx))<<std::endl; std::cout<<"expl_freq_der: "<<expl_freq_der<<std::endl;
        // std::cout<<"full freq der term: "<<expl_term*expl_freq_der*deltanu/dtau<<std::endl;
        // TODO: actually do the explicit part
        // std::cout<<"intensities before explicit part: "<<intensities(curr_point_on_ray_index,
        // curr_freq_idx)<<std::endl; std::cout<<"intensities after optical depth:
        // "<<intensities(curr_point_on_ray_index, curr_freq_idx)*exp(-dtau)<<std::endl;
        ...
        // std::cout<<"explicit part intensities: "<<intensities(rayposidx,
        // next_freq_idx)<<std::endl;
    }
    ...
    if (is_upward_disc) {
        ...
        for (Size next_freq_idx = model.parameters->nfreqs(); next_freq_idx > 0; next_freq_idx--) {
            ...
            // std::cout<<"intensities before implicit part: "<<intensities(rayposidx,
            // next_freq_idx-1)<<std::endl;
            ...
            // std::cout<<"implicit part intensities: "<<intensities(rayposidx,
            // next_freq_idx-1)<<std::endl;
        }
    } else {
        for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
            ...
            // std::cout<<"1.0-dIdnu coef times...: "<<1.0-dIdnu_coef1_next(rayposidx,
            // next_freq_idx)*deltanu/dtau<<std::endl;
            //
            // std::cout<<"intensities before implicit part: "<<intensities(rayposidx,
            // next_freq_idx)<<std::endl;
    ...
    if (closest_ray(rr, nextpointidx) == rayidx) {
        ...
        for (Size freqid = 0; freqid < model.parameters->nfreqs(); freqid++) {
            // Size1 corresponding_l_for_spec;           ///< number of line species corresponding
            // to frequency Size1 corresponding_k_for_tran;           ///< number of transition
            // corresponding to frequency Size1 corresponding_z_for_line;           ///< quadrature
            // number corresponding to frequency
            ...
            // std::cout<<"adding: "<<lspec.quadrature.weights[z] * wt * intensities(rayposidx,
            // freqid)<<std::endl; std::cout<<"intensity: "<<intensities(rayposidx,
            // freqid)<<std::endl;
            lspec.J(nextpointidx, k) +=
                lspec.quadrature.weights[z] * wt * intensities(rayposidx, freqid); // Su_()[centre];
...
inline void Solver ::solve_comoving_order_2_sparse(Model& model,
    ...
    // Vector<unsigned char>& real_pt= real_pt_();//for denoting whether the given point is a real
    // point
    Vector<Size>& nr = nr_();
    ...
    // This is due to the default computed shift mapping from static frame to comoving frame (I want
    // to do the reverse) for the backward ray, this should be correct (as we need to do the
    // reverse)
    ...
    // first_() = trace_ray_indicate_point(model.geometry, o, rr, dshift_max, -1, centre-1,
    // centre-1) + 1; last_ () = trace_ray_indicate_point(model.geometry, o, ar, dshift_max, +1,
    // centre+1, centre  ) - 1;
    ...
    // Note: technically, it does not matter which frame I use, as long as it is a fixed frame for
    // the entire ray
    ...
    if (closest_ray(rr, nr[first_()]) == rayidx) {
        ...
        for (Size freqid = 0; freqid < model.parameters->nfreqs(); freqid++) {
            // Size1 corresponding_l_for_spec;           ///< number of line species corresponding
            // to frequency Size1 corresponding_k_for_tran;           ///< number of transition
            // corresponding to frequency Size1 corresponding_z_for_line;           ///< number of
            // line number corresponding to frequency
            const Size unsorted_freqidx =
            ...
            LineProducingSpecies& lspec = model.lines.lineProducingSpecies[l];
            lspec.J(nr[first_()], k) +=
                lspec.quadrature.weights[z] * wt * intensities(first_(), freqid); // Su_()[centre];
    ...
    if (closest_ray(rr, nr[last_()]) == rayidx) {
        ...
        for (Size freqid = 0; freqid < model.parameters->nfreqs(); freqid++) {
            ...
            LineProducingSpecies& lspec = model.lines.lineProducingSpecies[l];
            lspec.J(nr[last_()], k) +=
                lspec.quadrature.weights[z] * wt * intensities(last_(), freqid); // Su_()[centre];
            // TODO: compute lambda term
        }
    }
    ...
    // from last_()-1 to first_ index, trace the ray backwards; Warning: rayposidx is an unsigned
...
accel inline void Solver ::solve_comoving_local_approx_order_2_sparse(Model& model,
    ...
    // Vector<Real>& cma_curr_frequencies=cma_curr_frequencies_();//will store the frequencies of
    // the current point for computing the explicit frequency derivative
    ...
    // compute_curr_opacity.fill(compute_curr_opacity.begin(), compute_curr_opacity.end(), 1); grr,
    // Vector is not a vector
    // manually fill instead
    for (Size freqidx = 0; freqidx < model.parameters->nfreqs(); freqidx++) {
        cma_compute_curr_opacity[freqidx] = true;
        // cma_curr_frequencies[freqidx] = model.radiation.frequencies.sorted_nu(nr_()[first_()],
        // freqidx);//in comoving frame
        // also compute starting intensity ;; err, why do I bother with a shift? It should be in the
        // frame of the point itself!!
        // cma_intensities[freqidx]=boundary_intensity(model, nr_()[first_()],
        // model.radiation.frequencies.sorted_nu(nr_()[first_()], freqidx)*shift_()[first_()]);
        cma_intensities[freqidx] = boundary_intensity(model, nr_()[first_()],
    ...
    if (closest_ray(rr, nr[first_()]) == rayidx) {
        ...
        for (Size freqid = 0; freqid < model.parameters->nfreqs(); freqid++) {
            ...
            LineProducingSpecies& lspec = model.lines.lineProducingSpecies[l];
            lspec.J(nr[first_()], k) +=
                lspec.quadrature.weights[z] * wt * cma_intensities[freqid]; // Su_()[centre];
            // TODO: compute lambda term; err, for bdy? technically zero as bdy condition is
            // enforced, so ignore
        }
    }
    // else
    ...
    // manually fill compute_curr_opacity, as we have not yet computed the current opacities on this
    // ray direction (dopper shift changing sign means we cannot reuse the last opacity on the
    // opposite direction)
    for (Size freqidx = 0; freqidx < model.parameters->nfreqs(); freqidx++) {
        cma_compute_curr_opacity[freqidx] = true;
        // cma_curr_frequencies[freqidx] = model.radiation.frequencies.sorted_nu(nr_()[last_()],
        // freqidx);//in comoving frame
        // also compute starting intensity // shift not necessary, as we are interested in the local
        // reference frame intensity
        // cma_intensities[freqidx]=boundary_intensity(model, nr_()[last_()],
        // model.radiation.frequencies.sorted_nu(nr_()[last_()], freqidx)*shift_()[last_()]);
...
// conditions is_upward_disc denotes whether we want an upward discretization nextpointonrayindex
// should denote how far the next point lies on the ray currpointonrayindex should do the same, but
// for the previous point//, const Size currpointonrayindex, const Size nextpointonrayindex
template <ApproximationType approx>
accel inline void Solver ::comoving_local_approx_map_data(Model& model, const Size curr_point,
    ...
    const Real dZ, const Size bdy_point) { //, const Size splitindex
    // if upward discretization, we start from the uppermost part
    bool is_in_bounds = false;
    // Size last_used_bdy_index = 0;//just for convenience sake when determining the explicit
    ...
    if (is_upward_disc) {
        ...
        Size curr_freq_idx = model.parameters->nfreqs() - 1 + 1;
        Size next_freq_idx = model.parameters->nfreqs() - 1 + 1;
        // const Real curr_sorted_nu = model.radiation.frequencies.sorted_nu(currpoint,
        // curr_freq_idx-1);
        ...
        const Size min_freq_idx = 0 + 1;
        // for determining whether the distance is large enough, just check whether the
        // corresponding line changes ERR, this does not work, unless we assume the lines to lie far
        // enough from eachother (utilize in single line approx) assumes at least a single frequency
        // will not be a boundary condition (is reasonable if we limit the doppler shift) NOTE: oob
        // freqs should be dealt with later on
        while ((curr_freq_idx >= (min_freq_idx + 1)) && next_freq_idx >= min_freq_idx) {
            ...
            if (model.radiation.frequencies.sorted_nu(curr_point, curr_freq_idx - 1) * curr_shift
                > model.radiation.frequencies.sorted_nu(next_point, next_freq_idx - 1)
                      * next_shift) {
                // //An implementation without break statement exists, but then we need to add some
                // extra helper boolean to signal that the next thing is oob. if
                // (curr_freq_idx==min_freq_idx)
                // {
                //     break;
                // }
                // more lazy implementation: put is_in_bounds_computation in last part
                curr_freq_idx--;
                // std::cout<<"curr_freq_idx"<<curr_freq_idx<<" min_freq_idx:
                // "<<min_freq_idx<<std::endl;
                ...
                // const bool branchless_if = (curr_freq_count ==
                // (encountered_lines.size()-1)*model.parameters->nquads()); is_in_bounds =
                // !branchless_if; in_bounds_count *= !branchless_if;//reset, as this will be used
                // for determining boundary during mapping
                   ...
                   // the previous point
            {
                ...
                // start_indices_()(nextpointonrayindex,next_freq_idx-1)[0]=currpointonrayindex;//stores
                // point on ray index
                // start_indices_()(nextpointonrayindex,next_freq_idx-1)[1]=curr_freq_idx-1;//stores
                // freq
                // for the sake of boundary conditions, these start_indices_ might be overwritten
                // later
                next_freq_idx--;
        ...
        while (next_freq_idx >= min_freq_idx) {
            ...
            //  const Real curr_freq = model.radiation.frequencies.sorted_nu(curr_point,
            //  curr_freq_idx-1)*curr_shift;//index - 1 due to size
            const Real curr_freq =
            ...
            comoving_approx_map_single_data<approx>(model, curr_point, next_point, dZ, is_in_bounds,
                in_bounds_count, next_freq_idx - 1, curr_freq_idx - 1, next_freq, curr_freq,
        ...
        while ((curr_freq_idx <= (max_freq_idx - 1)) && next_freq_idx <= max_freq_idx) {
            ...
            if (model.radiation.frequencies.sorted_nu(curr_point, curr_freq_idx) * curr_shift
                ...
                // An implementation without break statement exists, but then we need to add some
                // extra helper boolean to signal that the next thing is oob.
                ...
                curr_freq_idx++;
                // std::cout<<"curr_freq_idx: "<<curr_freq_idx<<" next_freq_idx:
                // "<<next_freq_idx<<"max_freq_idx: "<<max_freq_idx<<std::endl;
                   ...
                   // the previous point
            {
                ...
                // start_indices_()(nextpointonrayindex,next_freq_idx)[0]=currpointonrayindex;//stores
                // point
                // start_indices_()(nextpointonrayindex,next_freq_idx)[1]=curr_freq_idx;//stores
                // freq
                // for the sake of boundary conditions, these start_indices_ might be overwritten
                // later
                next_freq_idx++;
        ...
        while (next_freq_idx <= max_freq_idx) {
            ...
            comoving_approx_map_single_data<approx>(model, curr_point, next_point, dZ, is_in_bounds,
                in_bounds_count, next_freq_idx, curr_freq_idx, next_freq, curr_freq, next_shift,
                curr_shift, curr_line_idx, is_upward_disc, bdy_point);

            // start_indices_()(nextpointonrayindex,next_freq_idx)[0]=currpointonrayindex;//stores
            // point start_indices_()(nextpointonrayindex,next_freq_idx)[1]=max_freq_idx;//stores
            // freq
            next_freq_idx++;
            in_bounds_count++;
        }
    ...
    if (is_upward_disc) // determines the direction of the derivative too choose
    {
        ...
        // iterating over curr_freq_idx such that we later on can match using
        // cma_start_frequency_index
        for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
            ...
            // Real dfreqsmall = cma_start_frequencies_()[next_freq_idx+2] -
            ...
            const Real dfreqsmall =
                curr_freqp2 - curr_freqp1; // cma_start_frequencies_()[next_freq_idx+2] -
                                           // cma_start_frequencies_()[next_freq_idx+1];
            // (model.radiation.frequencies.nu(currpoint,
            // curr_freq_idxp2)-model.radiation.frequencies.nu(currpoint, curr_freq_idxp1))
            //           *shift_curr;
            const Real dfreqlarge =
                curr_freqp2 - curr_freq; // cma_start_frequencies_()[next_freq_idx+2] -
                                         // cma_start_frequencies_()[next_freq_idx+0];
            // (model.radiation.frequencies.nu(currpoint,
            // curr_freq_idxp2)-model.radiation.frequencies.nu(currpoint, curr_freq_idx))
            //           *shift_curr;
...
// Err, curr_line_idx is just some value that might or might not be adequate, as we may be
// connecting multiple lines together. Ignoring for now, as it only impact the single-line-approx
// (which fails in exactly the same situation of overlapping lines/very high doppler shifts)
template <ApproximationType approx>
accel inline void Solver ::comoving_approx_map_single_data(Model& model, const Size curr_point,
    ...
    if (!is_in_bounds || curr_freq_count < 2) {
        ...
        // overwrite starting intensities? Might be a bad idea, as this contains the computed
        // intensities
        ...
        cma_start_frequency_index_()[next_freq_idx] =
            is_upward_disc ? (model.parameters->nfreqs() - 1)
                           : 0; // dummy behavior, signaling that this is a bdy point for the
                                ...
                                // simplicity (otherwise I nedd another 'is_bdy' field)
        ...
        // so just compute using the line frequency? No, we might change from one line to the
        // next... TODO CHECK OTHER IMPLEMENTATION
        //🌴 other implementation did not in the slightest care about this, so ?
        // technically, it represents a maximal doppler shift, as any frequency matching will try to
        // reduce the shift. In that sense, we might just keep the (slightly inefficient lazy
        // implementation) Otherwise, we might just replace the delta
        // with 2.0*(next_freq-curr_freq)/(next_freq+curr_freq) (technically, the line frequency
        // should be used for the 2/Denominator part)
        ...
        // compute_curr_opacity; err, this might become cumbersome to store: simple algorithm: first
        // init to true, then save every result in the next freq list; finally get results from
        // freqmatched curr freq THE LISTS SHOULD BE SEPERATE to avoid overwriting; (note that we
        // probably may ignore the large velocity gradient branch for now, but we should program
        // this properly) and finally stuff to store (use next_freq_idx as index)
        Real Snext    = 0.0;
        Real Scurr    = 0.0;
        ...
        // cma_chi_curr_()[curr_freq_idx] = chi_curr;??is not part of default frequency
        // discretization cma_compute_curr_opacity_()[curr_freq_idx] = compute_curr_opacity;//if we
        // somehow encounter a current frequency index twice, we set this
        ...
        // should be used for the 2/Denominator part)
        // More correct would be: shift_c=1.0, shift_n = 1.0 + (next_freq-curr_freq)/line_freq
        // As we may use any frame of reference, we might as well put it a the current point in
        // order to decide whether the doppler shift is large
        const Real shift_c = curr_shift;
        const Real shift_n = next_shift;
        // const Real shift_c=1.0;
        ...
        cma_S_next_()[next_freq_idx] = Snext;
        cma_S_curr_()[next_freq_idx] = Scurr;
        // std::cout<<"snext: "<<cma_S_next_()[next_freq_idx]<<" scurr:
        // "<<cma_S_curr_()[next_freq_idx]<<std::endl;
        ...
        // nfreqs-1
        if (is_upward_disc) {
            // // //in order to circumvent annoying frequency derivative stuff bdy stuff, we
            // explicitly compute the freq derivative now
            ...
            // // const Size curr_freqp2 = (curr_freq_idx + 1 == model.parameters->nfreqs() - 1) ?
            // cma_start_frequencies_()[last_used_bdy_index]: curr_freq_idx+2;
            // // const Real curr_intensityp2 = cma_start_intensities_()[last_used_bdy_index]:
            // cma_computed_intensities_()[curr_freq_idx + 2];
            //
            // // Real dfreqsmall = cma_start_frequencies_()[next_freq_idx+2] -
            // cma_start_frequencies_()[next_freq_idx+1]; Real dfreqsmall =
            // cma_start_frequencies_()[next_freq_idx+2] -
            // cma_start_frequencies_()[next_freq_idx+1];
            // // (model.radiation.frequencies.nu(currpoint,
            // curr_freq_idxp2)-model.radiation.frequencies.nu(currpoint, curr_freq_idxp1))
            // //           *shift_curr;
            // Real dfreqlarge = cma_start_frequencies_()[next_freq_idx+2] -
            // cma_start_frequencies_()[next_freq_idx+0];
            // // (model.radiation.frequencies.nu(currpoint,
            // curr_freq_idxp2)-model.radiation.frequencies.nu(currpoint, curr_freq_idx))
            // //           *shift_curr;
            // cma_dIdnu_coef3_curr_()[next_freq_idx] =
            // -dfreqsmall/(std::pow(dfreqlarge, 2.0)-dfreqlarge*dfreqsmall);//farthest
            // cma_dIdnu_coef2_curr_()[next_freq_idx] =
            // dfreqlarge/(-std::pow(dfreqsmall, 2.0)+dfreqlarge*dfreqsmall);//nearer
            // cma_dIdnu_coef1_curr_()[next_freq_idx] =
            // -cma_dIdnu_coef3_curr_()[next_freq_idx]-cma_dIdnu_coef2_curr_()[next_freq_idx];//curr
            // point itself
            // //index stuff is actually not necessary, as for all points except the boundary, we
            // just need to increment the index.
            ...
            // (model.radiation.frequencies.nu(nextpoint,
            // next_freq_idx+2)-model.radiation.frequencies.nu(nextpoint, next_freq_idx+1))
            //             *shift_next;
            Real dfreqlarge = cma_end_frequencies_()[next_freq_idx + 2]
                            - cma_end_frequencies_()[next_freq_idx + 0];
            // (model.radiation.frequencies.nu(nextpoint,
            // next_freq_idx+2)-model.radiation.frequencies.nu(nextpoint, next_freq_idx))
            //             *shift_next;
            ...
            // std::cout<<"next_freq_idx: "<<next_freq_idx<<std::endl;
            // //Note: minus signs in front, as we are now computing the first derivative using
            // points on the other side (switched order of term, making sign clearer; just compare
            // to upward disc version) Real dfreqsmall = -(cma_start_frequencies_()[next_freq_idx-1]
            // - cma_start_frequencies_()[next_freq_idx-2]); std::cout<<"dfreqsmall:
            // "<<dfreqsmall<<std::endl;
            // // -(model.radiation.frequencies.nu(currpoint,
            // curr_freq_idxm1)-model.radiation.frequencies.nu(currpoint, curr_freq_idxm2))
            // // *shift_curr;
            // Real dfreqlarge = -(cma_start_frequencies_()[next_freq_idx-0] -
            // cma_start_frequencies_()[next_freq_idx-2]); std::cout<<"dfreqlarge:
            // "<<dfreqlarge<<std::endl;
            // // -(model.radiation.frequencies.nu(currpoint,
            // curr_freq_idx)-model.radiation.frequencies.nu(currpoint, curr_freq_idxm2))
            // // *shift_curr;
            // cma_dIdnu_coef3_curr_()[next_freq_idx] =
            // -dfreqsmall/(std::pow(dfreqlarge, 2.0)-dfreqlarge*dfreqsmall);//farthest
            // cma_dIdnu_coef2_curr_()[next_freq_idx] =
            ...
            // -(model.radiation.frequencies.nu(nextpoint,
            // next_freq_idx-1)-model.radiation.frequencies.nu(nextpoint, next_freq_idx-2))
            // *shift_next;
            Real dfreqlarge = -(cma_end_frequencies_()[next_freq_idx - 0]
                                - cma_end_frequencies_()[next_freq_idx - 2]);
            // -(model.radiation.frequencies.nu(nextpoint,
            // next_freq_idx)-model.radiation.frequencies.nu(nextpoint, next_freq_idx-2))
            // *shift_next;
            cma_dIdnu_coef3_next_()[next_freq_idx] =
                -dfreqsmall / (std::pow(dfreqlarge, 2.0) - dfreqlarge * dfreqsmall); // farthest
            cma_dIdnu_coef2_next_()[next_freq_idx] =
...
// start_indices_...
//  inline void Solver :: solve_comoving_single_step (Model& model, const Size raypos-idx, const
//  Size rayidx, const Size rr, const bool is_upward_disc, const bool forward_ray)
accel inline void Solver ::solve_comoving_local_approx_single_step(Model& model,
    ...
    // IF BORED: refactor this function such that it acts the explicit and implicit part on the same
    // time (not first forward for all freqs, than backward for all freqs)
    Vector<Size>& nr = nr_(); // stores the exact point indices
    // Vector<unsigned char>& real_pt=real_pt_();//stores whether each point is real point (not
    // added extra for interpolation purposes)
    ...
    // TODO: at end of this, do set the start equal to the computed intensities; THIS IS REQUIRED
    // FOR THE NEXT MAPPING STEP
    //*DONE*

    Vector<Real>& cma_dIdnu_expl = cma_dIdnu_expl_();
    // Vector<Real>& cma_dIdnu_coef1_curr=cma_dIdnu_coef1_curr_();//coefficient of the freq point
    // itself Vector<Real>& cma_dIdnu_coef2_curr=cma_dIdnu_coef2_curr_(); Vector<Real>&
    // cma_dIdnu_coef3_curr=cma_dIdnu_coef3_curr_();

    // Vector<Size>& cma_start_frequency_index=cma_start_frequency_index_();//direction will tell me
    // anyway which values to use (but do use modulo? to prevent oob access)
    ...
    // const Real next_shift=(forward_ray) ? 2.0-shift[rayposidx] : shift[rayposidx];//absolute
    // shifts, versus static frame!
    ...
    if (is_upward_disc) {
            ...
            + bdy_expl_term2
                  * cma_S_curr[model.parameters->nfreqs()
                               - 2]; // source term //no frequency derivative term, as the frequency
                                     // shift of the boundary is by definition 0
        // std::cout<<"bdy start I:
        // "<<cma_start_intensities[model.parameters->nfreqs()-1]<<cma_start_intensities[model.parameters->nfreqs()-2]<<std::endl;
        ...
        for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs() - 2;
            ...
            // const Size curr_point_idx=nr[curr_point_on_ray_index];
            // const Size curr_freq_idx=start_indices_()(rayposidx, next_freq_idx)[1];
            // const Real curr_shift=(forward_ray) ? 2.0-shift[curr_point_on_ray_index] :
            // shift[curr_point_on_ray_index];//absolute shift, versus static frame

            // std::cout<<"after computing shift"<<std::endl;

            // const Real deltanu=model.radiation.frequencies.nu(nextpointidx,
            // next_freq_idx)*next_shift-model.radiation.frequencies.nu(curr_point_idx,
            // curr_freq_idx)*curr_shift;
            const Real deltanu =
            ...
            // const Real
            // expl_freq_der=cma_dIdnu_coef1_curr[next_freq_idx]*cma_start_intensities[next_freq_idx]
            ...
            // std::cout<<"curr freq der coef 1: "<<dIdnu_coef1_curr(rayposidx,
            // next_freq_idx)<<std::endl; std::cout<<"curr freq der coef 2:
            // "<<dIdnu_coef2_curr(rayposidx, next_freq_idx)<<std::endl; std::cout<<"curr freq der
            // coef 3: "<<dIdnu_coef3_curr(rayposidx, next_freq_idx)<<std::endl;
            // std::cout<<"curr_point_on_ray_index: "<<curr_point_on_ray_index<<std::endl;
            // std::cout<<"respective freq index: "<<dIdnu_index1_curr(rayposidx,
            // next_freq_idx)<<std::endl; std::cout<<"intensity coef 1:
            // "<<intensities(curr_point_on_ray_index, dIdnu_index1_curr(rayposidx,
            ...
            cma_computed_intensities[next_freq_idx] =
                ...
                + expl_term * expl_freq_der * deltanu / dtau; // frequency derivative term
            // std::cout<<"explicit part intensities:
            // "<<cma_computed_intensities[next_freq_idx]<<std::endl;
            // Get the point indices
        }
    } else { //! is_upward_disc
        ...
        // std::cout<<"bdy start I:
        // "<<cma_start_intensities[0]<<cma_start_intensities[1]<<std::endl; for (Size
        // next_freq_idx=2; next_freq_idx<model.parameters->nfreqs(); next_freq_idx++)
        for (Size next_freq_idx = 2; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
    ...
    if (is_upward_disc) {
        ...
        cma_computed_intensities[model.parameters->nfreqs() - 1] =
            cma_computed_intensities[model.parameters->nfreqs() - 1]
            + impl_term1
                  * cma_S_next[model.parameters->nfreqs()
                               - 1]; // source term // no freq derivative term
        // +impl_term*impl_freq_der*deltanu/dtau)/(1.0-dIdnu_coef1_next(rayposidx,
        // next_freq_idx-1)*impl_term*deltanu/dtau); //freq derivative term
        const Real bdy_dtau2  = cma_delta_tau[model.parameters->nfreqs() - 2];
        ...
        for (Size next_freq_idx = model.parameters->nfreqs() - 2; next_freq_idx > 0;
            ...
            // const Real dtau=delta_tau(rayposidx, next_freq_idx);
            const Real dtau =
                cma_delta_tau[next_freq_idx - 1]; // std::cout<<"dtau: "<<dtau<<std::endl;
            // std::cout<<"next freq idx: "<<next_freq_idx-1<<std::endl;
            // std::cout<<"rayposidx: "<<rayposidx<<std::endl;
            const Real impl_term = (dtau + expm1(-dtau)) / dtau;
            // std::cout<<"impl term: "<<impl_term<<std::endl;
            ...
            // std::cout<<"implicit part intensities:
            // "<<cma_computed_intensities[next_freq_idx-1]<<std::endl;
        }
    } else {
    ...
    if (closest_ray(rr, nextpointidx) == rayidx) {
        ...
        for (Size freqid = 0; freqid < model.parameters->nfreqs(); freqid++) {
            ...
            //  const Size curr_freq_idx=start_indices_()(rayposidx, freqid)[1];
            //  const Real curr_shift=(forward_ray) ? 2.0-shift[curr_point_on_ray_index] :
            //  shift[curr_point_on_ray_index];//absolute shift, versus static frame const Real
            //  deltanu=model.radiation.frequencies.sorted_nu(nextpointidx,
            //  freqid)*next_shift-model.radiation.frequencies.sorted_nu(curr_point_idx,
            //  curr_freq_idx)*curr_shift;
    ...
    for (Size next_freq_idx = 0; next_freq_idx < model.parameters->nfreqs(); next_freq_idx++) {
        // note: for now, I might overwrite a bit too much stuff (which will be recomputed either
        // way) but better be safe than sorry for this complicated solver; I can always check later
        // which parts may safely be not overwritten
        /// these three things are not indexed using curr_freq_idx and are purely some helper vars
        /// for storing temp results; so these thing may not be overwritten
        // cma_start_intensities[next_freq_idx]=cma_computed_intensities[next_freq_idx];//is always
        // corrected when mapping the data itself
...
accel inline Size Solver ::trace_ray_comoving(const Geometry& geometry, const Size o, const Size r,
    ...
    if (geometry.valid_point(nxt)) {
        ...
        // if (closest_ray(rr, nxt)==rayidx)
        // {   //subtracting increment required, as id1 always points to the next point after
        // set_data
        //     outermost_interesting_point_rayidx = id1-increment;//as the data is set there, this
        //     should? be fine
        // }
    }
...
// accel inline bool Solver :: check_close_line (const Real prevfreq, const Real currfreq, const
// Real nextfreq, const Size prevpoint, const Size currpoint, const Size nextpoint, const Model&
// model)
{
