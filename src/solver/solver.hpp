...
struct Solver
{
    ...
    Matrix<Size> n_rays_through_point;//raydir, point ; might not be used that much in the future, as we might instead just use the closest ray for determining the result
    ...
    pc::multi_threading::ThreadPrivate<Matrix<Vector<Size>>> start_indices_;//For every point (on the ray) (maybe except the first point)), for every frequency, denotes from which index (pointonray, freq) one needs to start the computation
    // pc::multi_threading::ThreadPrivate<Matrix<std::array<Size, 2>>> start_indices_;//For every point (on the ray) (maybe except the first point)), for every frequency, denotes from which index (pointonray, freq) one needs to start the computation
    // This is necessary due to boundary conditions possibly refererring to points way back on the ray; NOTE: vectors have size 2
    //Done: use std::array<Size,2> instead of variable size Vector; errn never mind, need paracabs Vector to properly map to gpu
    ...
    //To figure out which lines overlap with eachother (unsigned char due to vector<bool> being a non-standard datatype)
    ...
    //TODO: use sorted lines to compute this instead: for two successive lines, check distance compared to max line width; if already large enough, do not bother checking more lines; otherwise check next line (this i due to the fact that a wide line might be 'hiding' behind a peaked line)
    ...
    //To make sure no 3 overlapping lines exist (with different line width), also check if the lines lie far enough from eachother to be non-overlapping (use same criterion as usual?)
    ...
    //For containing the boundary conditions more efficiently (for each line, one can then define some ranges of boundary frequencies to find overlap faster)
    ...
    // This needs to be done a priori, even though for default computation (no boundary conditions), it could be done on the fly.
    // The main reason is that we can (mis)use these coefficients to construct boundary conditions into the same formulation as any regular (non-bdy) computation.
    // Weirdly enough, our boundary conditions can be written as special cases of the comoving RTE.
