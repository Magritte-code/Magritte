...
inline void Image ::set_freqs(const Frequencies& frequencies) {
    ...
    // {
    //     throw std::runtime_error("The model currently does not have a spectral discretization for imaging. Thus the
    //     images cannot be initialized.")
    // }
...
/////////////////////////////////////////////////////////
// assumes the frequencies have already been set
inline void Image ::set_coordinates_projection_surface(
    const Geometry& geometry, const Size Nxpix, const Size Nypix) //(const Geometry& geometry)
{
    ...
    // Note: we instead use the coordinate system defined in set_coordinates_all_model_points, which is similar, in
    // order to be compatible with the previous imager.
    ...
    // Maybe in other function: define the imaging plane (do we still need the perpendicular vectors anywhere? I guess
    // not)
    if (geometry.parameters->dimension() == 1) {
        ...
        // In 1D, all model points 'lie' on the x-axis. Thus to get a proper image, we also need to use the same
    ...
    if (geometry.parameters->dimension() == 3) {
        ...
        if (denominator >= 1.0e-9) {
            ...
            // TODO: try to find paracabs fun that actually implements reduce operation (or use the ThreadPrivate stuff,
            // then manually min, max them)
            for (Size bdy_idx = 0; bdy_idx < geometry.parameters->nboundary(); bdy_idx++) {
                const Size bdy_point_index = geometry.boundary.boundary2point[bdy_idx];
...
accel Vector3D Image ::surface_coords_to_3D_coordinates(const double x, const double y) const {
    if (imagePointPosition != ProjectionSurface) {
        throw std::runtime_error(
            "Surface coordinates cannot be computed of image which does not define a projection surface.");
    }
    ...
    if (denominator >= 1.0e-9) {
        // To transform the distance in the direction back to the position, just multiply by the unit vector in that
        // direction
