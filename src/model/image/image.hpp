...
enum ImagePointPosition {AllModelPoints, ProjectionSurface};///< How to trace the imaging rays (either throughout all point or starting from a surface outside the model)
...
struct Image
{
    ...
    Image (const Geometry& geometry, const Frequencies& frequencies, const ImageType it, const Size ray_nr, const Size Nxpix, const Size Nypix);
    Image (const Geometry& geometry, const Frequencies& frequencies, const ImageType it, const Vector3D ray_dir);
    Image (const Geometry& geometry, const Frequencies& frequencies, const ImageType it, const Vector3D ray_dir, const Size Nxpix, const Size Nypix);
