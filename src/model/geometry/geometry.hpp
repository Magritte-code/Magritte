...
struct Geometry {
    ...
    accel inline Size get_next(const Vector3D& origin, const Vector3D& ray_dir, const Size crt,
        double& Z, double& dZ) const;

    template <Tracer tracer>
    accel inline Size get_next_general_geometry(const Vector3D& origin, const Vector3D& ray_dir,
        const Size crt, double& Z, double& dZ) const;

    accel inline Size get_next_spherical_symmetry(const Vector3D& origin, const Vector3D& ray_dir,
        const Size crt, double& Z, double& dZ) const;
    ...
    template <Frame frame>
    accel inline double get_shift(const Vector3D& origin, const Vector3D& origin_velocity,
        const Vector3D& raydir, const Size crt, const double Z, const bool reverse) const;
    ...
    accel inline double get_dist2_ray_point(const Size o, const Size p, const Size r) const;

    accel inline double get_dist2_ray_point_general_geometry(
        const Size o, const Size p, const Size r) const;

    accel inline double get_mean_dist_to_point(const Size o) const;
