...
print("nquads needed for static solver: ", tools.static_solver_nquads(1.0, 1.0, temp, temp, 0, 0, 0))
...
def run_model (nosave=False):
    ...
    timer5 = tools.Timer('static shortchar')
    timer5.start()
    ...
    error_u_stat = tools.relative_error (u_(x), u_stat[0,:,0])
    ...
    result += f'max error in static shortchar= {np.max(error_u_stat)}\n'
    result += f'--- Timers ------------------------------------\n'
    ...
    if not nosave:
        ...
        plt.scatter(x, u_stat[0,:,0], s=0.5, label='stat', zorder=1)
