...
nlspecs   = 1
# nquads    = 100
nquads    = 192; #=tools.static_solver_nquads(1.0, 1.0, temp, temp, 0, 0, vmax)
...
print("nquads needed for static solver: ", tools.static_solver_nquads(1.0, 1.0, temp, temp, 0, 0, vmax))
...
def run_model (nosave=False):
    ...
    model.compute_static_spectral_discretization ()
    ...
    timer5 = tools.Timer('setting model for static solver')
    timer5.start()
    model.compute_static_spectral_discretization ()
    model.compute_inverse_line_widths     ()
    model.compute_LTE_level_populations   ()
    ...
    timer6 = tools.Timer('static shortchar')
    timer6.start()
    model.compute_radiation_field_shortchar_static ()
    timer6.stop()
    u_stat = np.array(model.radiation.u)
    nu_stat = np.array(model.radiation.frequencies.nu)[0]
    ...
    us_stat = np.array([[[u_(f,r,a) for f in nu_stat] for r in rs] for a in angles])
    fs_stat = (nu_stat-frq)/frq*magritte.CC
    #err, nu_stat = nu if we are using the same frequencies at point 0
    ...
    error_u_stat = np.abs(tools.relative_error(us_stat, u_stat))
    ...
    result += f'mean error in static shortchar = {np.mean(error_u_stat)}\n'
    result += f'--- Timers --------------------------------------\n'
    ...
    # two different quadratures are used
    # plt.figure()
    # plt.plot(nu, nu_stat)
    # plt.show()
    ...
    if not nosave:
        ...
        plt.hist(error_u_stat.ravel(), bins=bins, histtype='step', label='stat')
        plt.xscale('log')
        plt.legend()
        ...
        plt.hist(error_u_stat.ravel(), bins=bins, histtype='step', label='stat', cumulative=True)
        plt.xscale('log')
        plt.legend()
        # plt.show()
