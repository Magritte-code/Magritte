...
def run_model (nosave=True):
    ...
    if not nosave:
        ...
        plt.figure(dpi=150)
        plt.title("wind feautrier")
        # plt.title("Constant opacity (optically thick)")
        plt.scatter(rs, Jstandard/abun, s=0.5, label='i=0', zorder=1)
        ...
        plt.ylabel('J [J/s/m^2]')

        plt.figure(dpi=150)
        plt.title("wind comoving")
        # plt.title("Constant opacity (optically thick)")
        plt.scatter(rs, Jcomoving/abun, s=0.5, label='i=0', zorder=1)
