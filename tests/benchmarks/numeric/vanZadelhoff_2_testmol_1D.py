...
def create_model (a_or_b):
    ...
    modelName = f'vanZadelhoff_2{a_or_b}_1D_testmol'
    ...
    model.chemistry.species.symbol    =  ['dummy0', 'pH2O', 'H2', 'e-', 'dummy1']
...
def run_model (a_or_b, nosave=False):
    ...
    timer3.start()
    model.compute_level_populations (True, 500)
    timer3.stop()

    J = np.array(model.radiation.J)
    print("J:", J)
    print("negative J's: ", J[J<0])
    emmis=np.array(model.lines.emissivity)
    print("emissivity: ", emmis)
    print("negative emmissivity: ", emmis[emmis<0])
    print("opacity: ", np.array(model.lines.opacity))
    ...
    print("lambda size: ", np.array(model.lines.lineProducingSpecies[0].Lambda.Ls))
    ...
    if not nosave:
        ...
        plt.scatter(rs, coll_cooling_rate, s=0.5, label='collisional', zorder=1)
        plt.scatter(rs, rad_cooling_rate, s=0.5, label='radiative', zorder=1)
        plt.legend()
        plt.xscale('log')
        plt.xlabel('r [m]')
        plt.yscale('log')
